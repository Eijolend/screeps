...
module.exports = {
	run : function(room){
		...
		if(room.controller.level > 3){
			civilian_target = Math.max(Math.min(Math.ceil(20/(bodies.civilian(maxEnergy).length/3)),7),2) + ( room.storage != undefined ? Math.floor(room.storage.store.energy/200000) : 0 );
		}
		var hunter_target = 0;
